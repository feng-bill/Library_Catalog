Public Class MemberSearch
    ...
    Private Sub accountBut_Click(sender As Object, e As EventArgs) Handles accountBut.Click
        If Login.loginSuccess = True Then
            Me.Hide()
            AccountReview.Show()
            AccountReview.Review.PerformClick()
        Else
    ...
    Private Sub CheckOutBut_Click(sender As Object, e As EventArgs) Handles CheckOutBut.Click
        If ResultsDataGrid.SelectedCells.Count = 0 Then
            MessageBox.Show("Please select the book you wish to Check out")
        Else
            Dim currentdate As Date = Date.Today.Date
            Dim availableShow As String = ResultsDataGrid.SelectedCells(5).Value.ToString

            If ResultsDataGrid.SelectedCells(5).Value.ToString.Equals("Available") = False Then
                MessageBox.Show("Your selected item is not Available")
            Else
                'When selected book is Available, make book unAvailable,
                'update check out date
                resourceid = ResultsDataGrid.SelectedCells(9).Value
                CheckoutPeriod = ResultsDataGrid.SelectedCells(8).Value
                DueDate = DateAdd(DateInterval.Day, CheckoutPeriod, currentdate)
                Resourcestatus = "Due " & DueDate
                If Login.loginSuccess Then
                    CheckoutTableAdapter.InsertCheckout(resourceid, Login.memberID, currentdate, DueDate)
                    ResultsDataGrid.SelectedCells(5).Value = Resourcestatus
                    ResultsDataGrid.SelectedCells(6).Value = currentdate
                    MessageBox.Show("you have successfully checkout your book.",
                        " Book checkout Successful", MessageBoxButtons.OK)
                Else
                    MessageBox.Show("You havent login yet. Go to home page and log in first.")
                End If
            End If
        End If
    End Sub
